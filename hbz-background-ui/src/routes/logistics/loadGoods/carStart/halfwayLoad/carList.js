...
const CarList = ({ dispatch, fields, tableTime, dataSource, selectedRowKeys, trackingNumberAll }) => {
  // 选中的方法
  ...
  // 不能选择的方法
  const disabledData = (record) => {
    for (let key of trackingNumberAll) {
      if (record.trackingNumber === key || record.trackingNumber === '合计') {
        return true
      } else {
        return false
      }
