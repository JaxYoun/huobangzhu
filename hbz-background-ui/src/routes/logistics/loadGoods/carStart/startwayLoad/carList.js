...
const CarList = ({ dispatch, fields, tableTime, dataSource, selectedRowKeys }) => {
  // 选中的方法
  const onChange = (value, data) => {
    dispatch({ type: 'carStart/updateCar', payload: { selectedRowKeys: value } })
    dispatch({ type: 'carStart/updateCar', payload: { alreadyChooseData: data } })
  }
  const columnsData = [
    {
      title: '物流编号',
  ...
  let rowSelections = {
    ...
    getCheckboxProps: record => ({
      disabled: record.trackingNumber === '合计',
    }),
  }
  return (
    <Card>
