...
const ChooseModal = ({
  ...
  const handleSubmit = () => {
    form.validateFields((err, formValue) => {
      ...
      dispatch({ type: 'carStart/updateChooseModal', payload: { loading: true } })
      dispatch({ type: 'carStart/uploadGoods', payload: formValue })
    })
  }
  // 关闭模态框
  const hideModal = () => {
    form.resetFields()
    dispatch({ type: 'carStart/updateChooseModal', payload: { visible: false, firmData: [], loading: false } })
  }
  // 全选
  const chooseAll = () => {
