...
const UnChooseModal = ({
  ...
  const handleSubmit = () => {
    form.validateFields((err, formValue) => {
      ...
      dispatch({ type: 'carStart/updateunChooseModal', payload: { loading: true } })
      dispatch({ type: 'carStart/unloadGoods', payload: formValue })
    })
  }
  // 关闭模态框
  const hideModal = () => {
    form.resetFields()
    dispatch({ type: 'carStart/updateunChooseModal', payload: { visible: false, firmData: [], loading: false } })
  }
  // 全选
  const chooseAll = () => {
