...
const ModalInfo = ({
  ...
  const hideModal = () => {
    form.resetFields()
    dispatch({ type: 'storageInfo/cleanModal' })
  }
  const handleSubmit = () => {
    form.validateFields((err, formValue) => {
      ...
      dispatch({ type: 'storageInfo/updatefirmModal', payload: { loading: true } })
      formValue.id = firmData.id
      dispatch({ type: 'storageInfo/updateInfo', payload: { id: firmData.id, warehouseDescribe: formValue.warehouseDescribe } })
      form.resetFields()
    })
  }
  return (
      ...
      >
        <Form>
          ...
          <Row className={styles.buttonPosition}>
            <Button type="primary" onClick={handleSubmit.bind(this)}>确定</Button>
...
const ModalInfoForm = Form.create()(ModalInfo)
