...
const UpShelfModal = ({
  ...
  const handleSubmit = () => {
    form.validateFields((err, formValue) => {
      ...
      dispatch({ type: 'scoreExchange/updateupShelfModal', payload: { loading: true } })
      formValue.wareId = firmData.ware.id
      formValue.id = firmData.id
      formValue.content = formValue.introduction
      dispatch({ type: 'scoreExchange/shelfUpGoods', payload: formValue })
      form.resetFields()
    })
  }
  // 关闭模态框
  const hideModal = () => {
    dispatch({ type: 'scoreExchange/cleanupShelfModal' })
    form.resetFields()
  }
  // 重置
  const cleanFirm = () => {
    form.resetFields()
    dispatch({ type: 'scoreExchange/updateupShelfModal', payload: { introductionData: '', imgUrlBig: '', imgUrlSmall: '', text: false } })
  }
      ...
      >
        ...
        <Form>
            ...
            <Col span={12}>
              <FormItem label="商品上架名称" {...formItemLayout}>
                {getFieldDecorator('productName', {
                  initialValue: firmData && firmData.productName,
            ...
            <Col span={12}>
              <FormItem label="商品类型" {...formItemLayout}>
                {getFieldDecorator('typeId', {
                  initialValue: (firmData && firmData.ware) && firmData.ware.typeId,
            ...
            <Col span={12}>
              <FormItem label="商品品牌" {...formItemLayout}>
                {getFieldDecorator('brandId', {
                  initialValue: firmData.id ? `${firmData.ware.brandId}` : '',
            ...
            <Col span={12}>
              <FormItem label="商品规格" {...formItemLayout}>
                {getFieldDecorator('specifications', {
                  initialValue: firmData && firmData.ware.specifications,
            ...
            <Col span={12}>
              <FormItem label="参考售价" {...formItemLayout}>
                {getFieldDecorator('amount', {
                  initialValue: firmData && firmData.ware.marketAmount,
          ...
          <Row>
            ...
            <Col span={12}>
              <FormItem label="兑换积分" extra="此处可能影响已有数据，请谨慎修改" {...formItemLayout}>
          ...
          <Row>
            <Col span={12}>
              <FormItem label="上架数量" {...formItemLayout}>
                {getFieldDecorator('total', {
                  ...
                  rules: [{ required: true, message: '请填写上架数量为非0正整数' }],
                })(
            ...
            <Col span={12}>
              <FormItem label="剩余数量" extra="此处可能影响已有数据，请谨慎修改" {...formItemLayout}>
                {getFieldDecorator('leave', {
                  initialValue: firmData && firmData.leave,
                  rules: [{ required: true, message: '请填写剩余数量为非0正整数' }],
                })(
          ...
          <Row>
            <Col span={24}>
              <FormItem label="商品详情" {...formItemLayouts}>
                ...
                <Text textshow={true} form={form} getHtmlData={introductionData || firmData.content} dispatch={dispatch} types="basicData/updateGoodsShelvesModal" />
              </FormItem>
            </Col>
          </Row>
          <Row>
...
const UpShelfModalForm = Form.create()(UpShelfModal)
export default UpShelfModalForm
