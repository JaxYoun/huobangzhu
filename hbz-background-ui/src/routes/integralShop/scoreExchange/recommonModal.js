...
const RecommonModal = ({
  ...
  // 关闭模态框
  const hideModal = () => {
    dispatch({ type: 'scoreExchange/cleanupRecommonModal' })
    form.resetFields()
  }
  // 提交
  const handleSubmit = () => {
    form.validateFields((err, formValue) => {
      ...
      dispatch({ type: 'scoreExchange/updateRecommonModal', payload: { loading: true } })
      formValue.productId = firmData.id
      dispatch({ type: 'scoreExchange/createRecommon', payload: formValue })
      form.resetFields()
    })
  }
  return (
            ...
            <Col span={12}>
              <FormItem label="商品编码" {...formItemLayout}>
                {getFieldDecorator('wareNo', {
                  initialValue: firmData && firmData.ware.wareNo,
          ...
          <Row>
            <Col span={12}>
              ...
              {getFieldDecorator('headerBit', {
                ...
                initialValue: imgUrl || firmData.header,
              })(
                <Input style={{ display: 'none' }} />
              )}
                <Avatar form={form}
                  imgUrl={imgUrl || firmData.header}
                  dispatch={dispatch}
                  type="scoreExchange/updateRecommonModal"
                  name="imgUrl"
                  nameForm="headerBit"
                />
              </FormItem>
...
const RecommonModalForm = Form.create()(RecommonModal)
export default RecommonModalForm
