...
const RegistProt = (props) => {
  const { visible, onCancel } = props
  const modalProps = {
    title: <span>《货帮主企业用户注册协议》</span>,
    visible,
    onCancel,
    footer: null,
    width: 800,
  ...
  return (
    <Modal {...modalProps} className={styles['protocal-modal']} >
      <div className={styles['protocal-box']}>
      　　尊敬的客户您好，欢迎您访问北京航数宽网科技有限责任公司网站（以下简称：网站）。在您注册成为网站会员之前，请您务必认真阅读和理解《网站用户注册协议》（以下简称：协议）中所有的条款。您须完全同意协议中所有的条款，才可以注册成为本网站的会员，使用里面的服务。您在网站的注册和操作均将被视为是您对协议所有条款及内容的自愿接受。
      　　第一条 声明
      　　（一）网站内在线产品的所有权归北京航数宽网科技有限责任公司所有。
      ...
      　　3、如果本网站发现您传输的信息含有本协议第二条所列内容之一的，本网站有权在不通知您的情况下采取包括但不仅限于向国家有关机关报告、保存有关记录、删除该内容及链接地址、关闭服务器、暂停您账号的操作权限、停止向您提供服务等措施；
      　　第三条 注册会员权利和义务
      　　（一）注册会员有权用本网站提供的服务功能。
