...
const EnterpriseConsignor = ({
  dispatch,
  personalCenter,
}) => {
  const { enterpriseConsignor } = personalCenter
  const { iDcardImage, licenceImage, registryData } = enterpriseConsignor
  const uploadProp = {
    title: '货主企业认证',
    headers: {
    ...
    registryData,
    dispatch,
    now: 'enterpriseConsignor',
  }
  return (
    <div>
      <Authentication {...uploadProp} dispatch={dispatch} />
    </div>
  )
}
EnterpriseConsignor.propTypes = {
  personalCenter: PropTypes.object,
  dispatch: PropTypes.func,
}
export default connect(({ personalCenter }) => ({ personalCenter }))(EnterpriseConsignor)
